...
public static class H3GeometryExtensions {
    ...
    /// <param name="result">optional result object; defaults to new <see cref="SqlGeography"/>
    /// instance.</param>
    ...
    /// <returns></returns>
    public static SqlGeography ToCoordinate(this H3Index inputIndex, SqlGeography? result = default, FaceIJK? toUpdateFaceIjk = default) {
        ...
        if (distance < EPSILON)
        {
            return SqlGeography.Point(center.LatitudeDegrees, center.LongitudeDegrees, 4326);
        }
        ...
        return SqlGeography.Point(latitude * M_180_PI, longitude * M_180_PI, 4326);
    }

    /// <summary>
    /// Convert a <see cref="SqlGeography"/> to a H3 index at the specified resolution.
    /// </summary>
    ...
    /// <returns></returns>
    public static H3Index ToH3Index(this SqlGeography coordinate, int resolution, FaceIJK? faceIjk = default, Vec3d? v3d = default) {
        if (resolution is < 0 or > MAX_H3_RES) return H3Index.Invalid;

        if (!coordinate.STIsValid()) return H3Index.Invalid;


        return H3Index.FromFaceIJK(
            FaceIJK.FromGeoCoord(
                coordinate.Long.Value * M_PI_180,
                coordinate.Lat.Value * M_PI_180,
                resolution,
                faceIjk,
                v3d
            ),
    ...
    public static SqlGeography GetCellBoundary(this H3Index index) {
        ...
        index.BuildCellBoundary(builder);

        return builder.ConstructedGeography;
    }

    public static void BuildCellBoundary(this H3Index index, SqlGeographyBuilder builder) {
        // get vertices and copy first onto the end to close the hole
        var polyVertices = GetCellBoundaryVertices(index).ToList();


        builder.BeginGeography(OpenGisGeographyType.Polygon);
        builder.BeginFigure(polyVertices[0].LatitudeDegrees, polyVertices[0].LongitudeDegrees);
        var nVertices = polyVertices.Count;
        for (var i = 1; i < nVertices; i++)
        {
            builder.AddLine(polyVertices[i].LatitudeDegrees, polyVertices[i].LongitudeDegrees);
        }
        builder.AddLine(polyVertices[0].LatitudeDegrees, polyVertices[0].LongitudeDegrees); //begin and end at same point
        builder.EndFigure();
    ...
    /// <returns></returns>
    public static SqlGeography GetCellBoundaries(this IEnumerable<H3Index> indices) {
